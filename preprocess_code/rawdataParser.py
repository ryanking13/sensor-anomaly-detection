...
# make aIndex.txt
with open('./csv_files/snu_label.csv') as csvfile:
	spamreader = csv.reader(csvfile, delimiter = ',' , quotechar = '|' )
	f = open('./data/aIndex.txt','w')
...
with open('./csv_files/snu_info.csv') as csvfile:
	spamreader = csv.reader(csvfile, delimiter = ',' , quotechar = '|' )
	for row in spamreader:
			if row[0] == "date_time" :
				continue

			waferNames.add(row[2])

			waferinfo = []

			waferinfo.append(row[2]) 				# name
			waferinfo.append( dateToInt(row[0]) )	# time in 0.01 sec format
			waferinfo.append(row[1])				# status
			waferinfo.append(row[4]) 				# stemnum 
...
for name in waferNames:
	...
	with open('./csv_files/snu_data2.csv') as csvfile:
		spamreader = csv.reader(csvfile, delimiter = ',' , quotechar = '|' )
		for row in spamreader:
			if row[0] == "date_time" :
				for i in range(2,27) :
	...
	with open('./csv_files/snu_data3.csv') as csvfile:
		spamreader = csv.reader(csvfile, delimiter = ',' , quotechar = '|' )
		for row in spamreader:
			if row[0] == "date_time" :
				for i in range(2,26) :
